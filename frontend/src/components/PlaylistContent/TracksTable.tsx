  ...
  return (
    <div>
      <div className="grid items-center grid-cols-[16px_400px_1fr_1fr_80px] gap-4 px-4 py-2 border-b border-white/10 mb-2">
        <span className="text-subdued text-10-medium font-medium">#</span>
        <span className="text-subdued text-10-medium font-medium">
          Título
        </span>
        <span className="text-subdued text-10-medium font-medium">
          Álbum
      ...
      {playlist.musics.map((music, index) => (
        <TrackTableRow key={music.id} music={music} index={index} />
      ))}
    </div>
  )
}